...
whenever he goes to eat in a restaurant. 
In his country, it's usual to tip 15% 
...
if...else statement, and then try to convert it 
to a ternary operator).

Print a string to the console containing the bill 
...
console.log(`---------- ternory oprator ----------`);
const bill = 275;
const tip = bill >= 40 && bill <= 300 ? bill * 0.15 : bill * 0.2;
...
if (bill >= 50 && bill <= 300) {
  console.log(
    `the bill Value ${bill}, the tip ${bill * 0.15}, and the final value (${
      bill + bill * 0.15
    }).`
  );
} else {
  console.log(
    `the bill Value ${bill}, the tip ${bill * 0.2}, and the final value (${
      bill + bill * 0.2
    }).`
  );
}
