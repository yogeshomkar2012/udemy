...
const checkDogs = (juliaDogs, kateDogs) => {
  let dogsJuliaCorrect = juliaDogs.slice();
  dogsJuliaCorrect.splice(0)
  dogsJuliaCorrect.splice(-2)
  const dogs = dogsJuliaCorrect.concat(kateDogs);

  dogs.forEach((dogs, i) => {
    if (dogs >= 3) {
      console.log(`Dog number ${i + 1} is an adult and is ${dogs} years 
    old`);
    } else {
      console.log(`Dog number ${i + 1} is still a puppy ${dogs}🐶`);
    }
  });
};
